...
### Changed
- **OpenClaw baseline moved to `2026.7.1-2`** — plugin peer range is now `>=2026.7.1-2 <2027`. **BREAKING:** support for `2026.5.28`–`2026.7.1-1` and `extended-stable` `2026.6.33` is dropped; OMM tracks a single baseline (OpenClaw `latest`). Consumers on an older host stay on the previous plugin release (`@oh-my-matrix/autopilot@3.1.0` / `@oh-my-matrix/dynamic-workflows@0.2.0` on npm + matching git tag). The `-2` floor (not `>=2026.7.1`) is required because semver excludes the `-N` correction prerelease from a plain range. devDep/test baseline pinned `openclaw@2026.7.1-2`. SDK drift: `PluginHookBeforeToolCallEvent` gained optional `toolKind`/`toolInputKind`/`derivedPaths` (openclaw 2026.7.1); stale "NO toolKind" claims refreshed across contracts, both `index.ts` hooks, the subagent-guard test header, and `docs/fixes/runtime-guard-event-shape.md` (new fields unused — `decidePermissionForEvent` does not forward `toolKind`). See `.changeset/openclaw-2026-7-1-2.md`.
