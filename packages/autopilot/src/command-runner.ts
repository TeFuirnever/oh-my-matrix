...
export async function runValidationCommands(
  ...
  for (const cmd of commands) {
    ...
    try {
      ...
      const e = err as { killed?: boolean; code?: number | string; signal?: string; stderr?: string; message?: string };
...
function isCmdLike(bin: string): boolean {
  const lower = bin.toLowerCase();
  return lower.endsWith('.cmd') || lower.endsWith('.bat');
}

function shouldUseShell(bin: string, args: string[], cwd: string | undefined): boolean {
