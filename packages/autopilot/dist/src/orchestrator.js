...
function orchestratorReducer(state, event) {
    switch (event.type) {
            ...
            if (!(0, retry_queue_1.shouldRetry)({ attempt: currentAttempt, maxRetries, recoverable: classification.recoverable })) {
                ...
                return {
                    ...
                    blockedReason: classification.recoverable
                        ? 'max_retries_reached'
                        : (0, types_1.toBlockedReason)(event.error ?? 'unknown error'),
                    lastActivityAt: event.now,
                };
            }
            // Recoverable → retry_queued
            const retryEntry = (0, retry_queue_1.buildRetryEntry)(currentAttempt, event.error ?? 'unknown', event.now, maxRetryBackoffMs);
            return {
                ...state,
        ...
        case 'stall_timeout': {
            ...
            const retryEntry = (0, retry_queue_1.buildRetryEntry)(currentAttempt, 'stalled', event.now, maxRetryBackoffMs);
            return {
                ...state,
                orchestrationState: 'retry_queued',
                retry: retryEntry,
