...
describe('permission-wiring: subagent runtime guard (Design 2)', () => {
  // Subagent sessions (OpenProse-spawned workflow branches) are NOT autopilot
  // runs and previously fell through the before_tool_call gate with no enforcement.
  // Design 2 enforces fail-closed destructive-op blocking at runtime for every
  // :subagent: session, closing the prompt-CHECKPOINT bypass weak models have.
  let mock: ReturnType<typeof createMockApi>;
  ...
  it('blocks destructive_git for a subagent session with no autopilot run', async () => {
    const beforeToolCall = mock.hooks.get('before_tool_call')!;
    const result = await beforeToolCall(
      { toolName: 'git', toolKind: 'destructive_git', args: ['reset', '--hard', 'HEAD~1'] },
  ...
  it('allows read-only tools for a subagent session', async () => {
    ...
    expect(result).toBeUndefined(); // undefined = allowed (pass)
  });
  ...
  it('does NOT enforce on the main session (no :subagent:, no autopilot run)', async () => {
    // A non-subagent, non-autopilot session keeps normal behavior (early return).
    const beforeToolCall = mock.hooks.get('before_tool_call')!;
    const result = await beforeToolCall(
