    ...
    for (const seg of segments) {
        if (GIT_BINARIES.has(seg[0]?.toLowerCase() ?? '')) {
            ...
            for (let i = 1; i < seg.length; i++) {
                ...
                if (seg[i].startsWith('-C') && seg[i].length > 2) {
                    cwd = seg[i].slice(2);
                    break;
                }
            }
...
function classifyCommand(tool, args = [], toolKind) {
    ...
    if (toolLower === 'git' && args.length > 0) {
        ...
        if (sub === 'checkout') {
            if (args.includes('--'))
                return 'destructive_git'; // explicit discard separator
            const target = args[idx + 1];
            if (target === '.' || target === '*')
                return 'destructive_git'; // discard workdir changes
        }
        ...
        if (sub === 'branch' && args.some(a => a === '-D' || a === '-d' || a === '--delete'))
            return 'destructive_git';
        if (sub === 'tag' && args.some(a => a === '-d' || a === '--delete'))
            return 'destructive_git';
        if (sub === 'stash' && args.slice(idx + 1).some(a => a === 'clear' || a === 'drop'))
            return 'destructive_git';
