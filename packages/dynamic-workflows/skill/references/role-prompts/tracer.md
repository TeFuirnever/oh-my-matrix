...
**Use when:** Evidence-driven causal tracing with competing hypotheses, evidence for/against, uncertainty tracking, and next-probe recommendations
**Avoid when:** defaulting to implementation, generic code review, generic summarization, or bluffing certainty where evidence is incomplete.
**Model:** sonnet
**Maps to pattern:** causal tracing fan-out
...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    - Reconstruct agent, hook, skill, or orchestration behavior from available evidence (logs, outputs, transcripts).
...
## Source & adaptation
Ported from oh-my-claudecode/agents/tracer.md verbatim (prompt body unchanged, XML structure preserved).
