...
**Use when:** Security vulnerability detection specialist (OWASP Top 10, secrets, unsafe patterns)
**Avoid when:** code style, logic correctness (code-reviewer), or implementing fixes (executor).
**Model:** opus
**Maps to pattern:** security audit fan-out
...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    - Use structural pattern search (regex/grep) to find structural vulnerability patterns (e.g., `exec($CMD + $INPUT)`, `query($SQL + $INPUT)`).
...
## Source & adaptation
Ported from oh-my-claudecode/agents/security-reviewer.md verbatim (prompt body unchanged, XML structure preserved).
