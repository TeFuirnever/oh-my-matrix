#!/usr/bin/env bash
# sync-to-ma.sh — refresh MA's @omm/dynamic-workflows from this source.
#
...
# 2. resolve version + tgz name (@omm/dynamic-workflows → omm-dynamic-workflows-<ver>.tgz)
VERSION="$(node -p "require('./package.json').version")"
TGZ="omm-dynamic-workflows-${VERSION}.tgz"
...
find "$MA_RES_DIR" -name 'omm-dynamic-workflows-*.tgz' ! -name "$TGZ" -print -delete || true

# 4. update MA package.json file: ref (sed preserves formatting)
WANT="file:resources/dynamic-workflows/${TGZ}"
sed -i.bak -E "s|file:resources/dynamic-workflows/omm-dynamic-workflows-[0-9]+\.[0-9]+\.[0-9]+\.tgz|${WANT}|" "$MA_ROOT/package.json" && rm -f "$MA_ROOT/package.json.bak"
...
echo "✓ @omm/dynamic-workflows ${VERSION} synced to MA"
