...
| **v16** | **运行时守卫落地（根治弱模型绕过）**：B1 经 3 轮 ralplan 共识演进——ADR-011 守卫先 ship 在 autopilot plugin（Design 2，subagent fail-closed，检测 `:subagent:` session）；ADR-012 提取为独立 `@openclaw/dynamic-workflows` plugin（`before_tool_call` priority 11，先于 autopilot 10 / audit 9 跑，block 短路）；ADR-013 进一步把原语（decidePermission/classifyCommand/audit）抽成中性 `@openclaw/permission-policy` 库，autopilot 与 dynamic-workflows 互不依赖。SKILL.md 加运行时强制说明（指向 dynamic-workflows plugin）。弱模型无法绕过（检测基于 sessionKey，不靠 agent 配合）。组件 641 测试绿；MA live e2e runbook 见 `docs/runbooks/ma-subagent-guard-e2e.md`。| **当前（实施态）** |
...
>
> **已实施 + 演进（ADR-011→012→013）**：运行时守卫现住在独立的 `@openclaw/dynamic-workflows` plugin（`before_tool_call` priority 11），检测 `:subagent:` session（OpenProse spawn 的 workflow 分支），以 `decidePermission(workflowAllowsDestructiveGit:false)` fail-closed 拦截 destructive git / credential_access / system_write。原语（decidePermission/classifyCommand/audit）现住在中性 `@openclaw/permission-policy` 库——autopilot 与 dynamic-workflows 都消费它、互不依赖。**完全运行时、不依赖 prompt、弱模型无法绕过**（检测基于 sessionKey 约定，不靠 agent 配合）。合法 destructive git 走 autopilot run + WORKFLOW.md `destructive_git.allow=true` 路径。详见 ADR-011/012/013 与 `packages/dynamic-workflows/index.ts`。
...
|---|------|------|---------|---------|-----------|
| B1 | ~~Skill-level hooks~~ → **✅ 已落地（运行时守卫，ADR-011→012→013）** | OpenClaw | ❌ skill-level frontmatter hooks 证伪（oh-my-manus 自述 OpenClaw 不执行 `hooks:`）；但"ROI 不足"前提错（`packages/autopilot/` 已有 `before_tool_call`+`decidePermission`）。**已实施并演进**：ADR-011 守卫 ship 在 autopilot（Design 2）；ADR-012 提取为 `@openclaw/dynamic-workflows` plugin（priority 11）；ADR-013 原语抽成 `@openclaw/permission-policy` 中性库，两 plugin 解耦、互不依赖。检测 `:subagent:` session → fail-closed 拦截 destructive git，弱模型不可绕过 | dynamic-workflows plugin + permission-policy 库 | **✅ 已实施** |
