...
| **v16** | **运行时守卫落地（根治弱模型绕过）——⚠ 但发现 fail-open bug，待修**：B1 经 3 轮 ralplan 共识演进——ADR-011 守卫 ship 在 autopilot plugin（Design 2）；ADR-012 提取为 `@openclaw/dynamic-workflows` plugin（priority 11）；ADR-013 原语抽成 `@openclaw/permission-policy` 库，两 plugin 解耦。**但 2026-06-27 对抗审查发现 P0 bug**：守卫读 `event.args`/`event.toolKind`，真实事件是 `params` 对象 + `toolKind:"code_mode_exec"`（唯一值）→ 生产中 fail-open（destructive git 被放行）；测试用虚构事件形状所以"绿"。**架构（解耦/优先级/库）sound，事件管道 plumbing 坏**。bug 是 pre-existing（MA autopilot）。修复 spec：`docs/fixes/runtime-guard-event-shape.md`（NOT STARTED）。| **当前（实施态，含待修 bug）** |
...
>
> **已实施 + 演进（ADR-011→012→013）——⚠ 但 2026-06-27 审查发现 fail-open bug（待修）**：运行时守卫现住在独立的 `@openclaw/dynamic-workflows` plugin（`before_tool_call` priority 11），原语在中性 `@openclaw/permission-policy` 库，autopilot 与 dynamic-workflows 互不依赖。**但守卫读 `event.args`/`event.toolKind`，真实 OpenClaw 事件是 `params` 对象 + `toolKind:"code_mode_exec"`（唯一值）→ 生产中 fail-open（destructive git 被放行），测试因用虚构事件形状而"绿"。** 架构 sound，事件管道 plumbing 坏（pre-existing，来自 MA autopilot）。修复 spec：`docs/fixes/runtime-guard-event-shape.md`（NOT STARTED）。详见 ADR-011/012/013（含 bug 标注）。
