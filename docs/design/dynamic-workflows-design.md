...
| **v16** | **运行时守卫落地（根治弱模型绕过）——✅ fail-open bug 已修（2026-06-28）**：B1 经 3 轮 ralplan 共识演进——ADR-011 守卫 ship 在 autopilot plugin（Design 2）；ADR-012 提取为 `@oh-my-matrix/dynamic-workflows` plugin（priority 11）；ADR-013 原语抽成 `@oh-my-matrix/permission-policy` 库，两 plugin 解耦。**2026-06-27 对抗审查发现 P0 bug**（守卫读 `event.args`/`event.toolKind`，真实事件是 `params` 对象 → 生产 fail-open；测试用虚构事件形状所以"绿"），**2026-06-28 修复并验证**：真实事件形状 live capture、测试改真实形状、evasion 路径封堵（force-push / `&` background / `$(...)` substitution / `npx` wrapper-exec）、编译期事件形状契约、部署 dist 直驱验证（deployed-dist smoke check, run in the host repo）。残留 tokenize-based 已知局限见 fix spec。修复 spec：`docs/fixes/runtime-guard-event-shape.md`（DONE）。| 实施态 |
| **v17** | **角色库迁移：14 个自写角色 → oh-my-claudecode 19 个 agent（原样移植，2026-08-10）**：`references/role-prompts/` 全部替换——prompt 正文 verbatim（XML 结构保留），仅剥离 OMC-only 引用/工具/`.omc/` 路径 + 补只读 posture 声明；删 8 个 OMM 自写角色（skeptic/judge/reviewer/implementer 等）。patterns/templates 同步换角色（skeptic→critic、judge→critic、contestant→executor、finder→tracer、drafter→writer、investigator→scientist@opus）+ 词表统一（REFUTED/SURVIVES + critic/code-reviewer 映射段）+ Step 3 占位符强制检查 + fan-out per-item sonnet override。5 轮对抗 review（41+3+14 项）驱动：OMC 字眼清零、refute 词表一致、只读/可写分类修正（scientist 走 Bash 写报告）。宿主分发源 `resources/skills/default/dynamic-workflows/` 已同步。| 实施态 |
| **v18** | **通用场景触发机制（2026-08-11）**：两层触发杠杆落地——(1) SKILL.md description 加 "use proactively" + 开发/任务处理场景触发词（EN+ZH）；(2) 插件 `agent_turn_prepare` hook（priority 12，主会话）确定性预筛——读 `event.prompt`，信号词/规模阈值/小任务抑制词判定，命中返回 `appendContext` fan-out 指引（与 autopilot goal 注入 concat 共存）；跳过 `:subagent:` 分支。`isFanOutCandidate` 纯函数 + e2e 真实事件形状测试。研究结论：业界无触发概率机制，唯一杠杆是 hook 确定性 + description 语义匹配。同时落地 B6 `validate-prose.mjs`（OpenProse 不可用时的独立 5-check 验证）。| **当前（实施态）** |
...
>
> **已实施 + 演进（ADR-011→012→013）——✅ fail-open bug 已修（2026-06-28）**：运行时守卫住在独立的 `@oh-my-matrix/dynamic-workflows` plugin（`before_tool_call` priority 11），原语在中性 `@oh-my-matrix/permission-policy` 库，autopilot 与 dynamic-workflows 互不依赖。守卫读真实事件 `params.command`（按 `&&`/`&`/`|`/`;`/换行 拆段逐段分类），subagent 会话 defaultDeny fail-closed，evasion（force-push / amend / `$(...)` substitution / `npx` wrapper-exec）封堵；编译期事件形状契约防漂移；部署 dist 直驱验证（deployed-dist smoke check, run in the host repo）。残留 tokenize-based 已知局限（redirect 写 / defaultDeny non-shell / quote split）见 fix spec。修复 spec：`docs/fixes/runtime-guard-event-shape.md`（DONE）。详见 ADR-011/012/013。
...
|---|------|------|---------|---------|-----------|
| B1 | ~~Skill-level hooks~~ → **✅ 已落地（运行时守卫，ADR-011→012→013）** | OpenClaw | ❌ skill-level frontmatter hooks 证伪（oh-my-manus 自述 OpenClaw 不执行 `hooks:`）；但"ROI 不足"前提错（`packages/autopilot/` 已有 `before_tool_call`+`decidePermission`）。**已实施并演进**：ADR-011 守卫 ship 在 autopilot（Design 2）；ADR-012 提取为 `@oh-my-matrix/dynamic-workflows` plugin（priority 11）；ADR-013 原语抽成 `@oh-my-matrix/permission-policy` 中性库，两 plugin 解耦、互不依赖。检测 `:subagent:` session → fail-closed 拦截 destructive git，弱模型不可绕过 | dynamic-workflows plugin + permission-policy 库 | **✅ 已实施** |
...
| B6 | ~~`scripts/validate-prose.sh`~~ → **✅ 已实施（`packages/dynamic-workflows/scripts/validate-prose.mjs`，2026-08-11，commit bf11d23）** | OpenClaw | ✅ 已落地 | 无 | node 脚本（5 check 自动化 + 8 单测） |
