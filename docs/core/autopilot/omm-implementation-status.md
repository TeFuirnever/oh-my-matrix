...
> 📌 **发版事实**：autopilot `4.5.0` + instinct `0.2.0`（2026-09-19，tag 齐全）；instinct `0.3.0` 合入但**从未发布 npm**（tarball 缺 `openclaw.plugin.json`，0.2.0/0.3.0 从 npm 均不可安装）；`0.3.1`（host 契约 + 打包修复，首个可安装版）与 `0.3.2`（截断安全去重）于 09-19/20 发布；autopilot `4.5.1`（crash-recovery 广播）/ `4.5.2`（`ledger:{}` 崩溃循环，ticket-13）/ `4.5.3`（非对象 folded 双层 spread 守卫，ticket-13 追加，MA cross-review 驱动）2026-09-20/21 发布——MA 已 bump 4.5.3（!3254 head `d29fde27e`，178/178 绿 + smoke 断言版 ✅），**X3 全链（恢复+广播+ledger+junk 守卫）跨仓闭环**。registry 验证全过。
