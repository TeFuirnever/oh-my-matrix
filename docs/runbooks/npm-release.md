...
4. **host smoke(autopilot 前置 gate)**—— 真实 openclaw 宿主装本次版本跑一轮(注册、hook/RPC 表面、activate 后 destructive blocked)。仓库测试绿 ≠ host 已生效(placebo bug 教训,host-deploy.md §5)。**2026-08-09 4.0.0 漏做此步、已补跑通过**(MA `scripts/smoke-plugin-runtime.mjs`;2026-08-18 核实:该脚本只在 MA 悬空 commit `ddb6246e` 上,未合入分支,当前不可用,恢复见 issue #171);发布 gate 顺序:host smoke **在** publish 之前。host 环境在消费方(MA 侧)。
