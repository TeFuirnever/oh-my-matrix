...
1. **bump 版本** `packages/<pkg>/package.json`,commit 用 conventional:`chore(<pkg>): bump to X.Y.Z`。
2. **CHANGELOG** 记一条该版本变更。
3. **CI 绿**(`pnpm verify` + CI 全过)再发。
4. **npm publish** —— 真实终端,2FA OTP。见 memory `npm-publish-flow`(agent 跑不了 `npm publish`)。
