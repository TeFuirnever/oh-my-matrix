...
- This repository is **documentation & design first**. It also hosts three canonical pnpm workspace packages: `@oh-my-matrix/autopilot` (v3.0.0), `@oh-my-matrix/dynamic-workflows` (v0.1.2), and `@oh-my-matrix/permission-policy` (v0.1.1) — see [ADR-010](docs/adr/010-autopilot-source-hosting.md). The v0.x OpenClaw plugin / MCP implementation has been removed and its design records archived under `docs/archive/`.
- `packages/autopilot/` is **hosted** source (ADR-010), not an omm deliverable. After source changes run `corepack pnpm -r test && corepack pnpm -r typecheck`, then run the internal host-deploy step (not in this repo) to rebuild, refresh the host's tgz, install, and rebuild the bundled-plugin copy the Gateway loads.
