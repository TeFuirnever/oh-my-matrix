...
- This repository is **documentation & design first**. It also hosts four canonical pnpm workspace packages published to npm: `@oh-my-matrix/autopilot`, `@oh-my-matrix/dynamic-workflows`, `@oh-my-matrix/permission-policy`, and `@oh-my-matrix/instinct` — see [ADR-010](docs/adr/010-autopilot-source-hosting.md). Current versions are in each package's `CHANGELOG.md` or on the [npm registry](https://www.npmjs.com/org/oh-my-matrix) (do not hardcode version numbers here — they drift on every release). The v0.x OpenClaw plugin / MCP implementation has been removed and its design records archived under `docs/archive/`.
- `packages/autopilot/` holds the **canonical** autopilot source (ADR-010). It is published to the npm registry as `@oh-my-matrix/autopilot` via Changesets + `scripts/publish.sh` (see CONTRIBUTING.md § Releasing). After source changes run `corepack pnpm -r test && corepack pnpm -r typecheck`; a change is **not live** in a consuming host until publish + host version bump + gateway restart + deployed-dist smoke check (see `docs/architecture.md` § Distribution Reality).
- Changes to the **subagent runtime guard** (`permission-policy` / `dynamic-workflows` / `autopilot` before_tool_call) ship via the internal host-deploy step (builds all 4 packages + cp dist into the host); verify the deployed dist with a deployed-dist smoke check (run in the host repo) before restarting the host gateway. See `docs/fixes/runtime-guard-event-shape.md`.
