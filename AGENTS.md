...
- `packages/autopilot/` is **hosted** source (ADR-010), not an omm deliverable. After source changes run `pnpm --filter @oh-my-matrix/autopilot test`, then run the internal host-deploy step (not in this repo) to rebuild, refresh the host's tgz, install, and rebuild the bundled-plugin copy the Gateway loads.
