...
| `@oh-my-matrix/instinct` | 跨会话 context 记忆。`after_tool_call` 观测（脱敏）→ `.instinct/observations.jsonl`；`instinct_record` tool 让主 agent 记录 working pattern → `.instinct/instincts.jsonl`（精确去重 + hits）；`agent_turn_prepare` 首轮注入两段 recall（raw 尾部 + instincts），`session_start` 做双族 30 天 purge（10 MB rotation / secret scrub） | ✅ shipped source | [`packages/instinct/`](packages/instinct/) |
...
| npm public package | 已发布 `@oh-my-matrix/*`（autopilot / dynamic-workflows / permission-policy）—— 当前版本见各包 [CHANGELOG](packages/autopilot/CHANGELOG.md) 或 [npm registry](https://www.npmjs.com/org/oh-my-matrix) |
| CI / Harness | GitHub Actions 6 gate（lint / typecheck / commitlint / test / hook-priority / audit）+ 本地 `pnpm verify` 镜像 |
