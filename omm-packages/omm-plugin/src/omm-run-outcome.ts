...
const VALID_KINDS: ReadonlySet<RunOutcomeKind> = new Set([
  "completed",
  "failed",
  "blocked",
  "cancelled",
...
const PHASE_TO_KIND: Record<string, RunOutcomeKind> = Object.fromEntries(
  Object.entries(KIND_TO_PHASE).map(([kind, phase]) => [
    phase,
    kind as RunOutcomeKind,
  ]),
);
 ...
 * them.
 */
export function phaseToOutcomeKind(phase: string): RunOutcomeKind | null {
  return PHASE_TO_KIND[phase.trim().toLowerCase()] ?? null;
}

/** Inverse of phaseToOutcomeKind — the phase string for a given outcome kind. */
export function outcomeKindToPhase(kind: RunOutcomeKind): string {
...
export function isRunOutcome(value: unknown): value is RunOutcome {
  ...
  if (typeof v.kind !== "string" || !VALID_KINDS.has(v.kind as RunOutcomeKind))
    return false;
  if (
    typeof v.mode !== "string" ||
    !VALID_MODES.has(v.mode as RunOutcome["mode"])
  )
    return false;
  if (typeof v.finishedAt !== "string") return false;
  if (!Number.isFinite(Date.parse(v.finishedAt))) return false;
...
export function deriveOutcomeFromState(
  ...
  if (
    typeof mode !== "string" ||
    !VALID_MODES.has(mode as RunOutcome["mode"])
  ) {
    return null;
  }
