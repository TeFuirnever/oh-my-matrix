...
const TIERS: ReadonlySet<AgentModelTier> = new Set(["haiku", "sonnet", "opus"]);
 ...
 */
export function parseAgentPrompt(raw: string): AgentPrompt {
  ...
  if (!name || !NAME_PATTERN.test(name)) {
    throw new Error(
      `agent prompt name must match /^[a-z][a-z0-9-]*$/, got: ${name ?? "(missing)"}`,
    );
  }
  const tierRaw = fields.model_tier as AgentModelTier | undefined;
  if (!tierRaw || !TIERS.has(tierRaw)) {
    throw new Error(
      `agent prompt model_tier must be one of haiku|sonnet|opus, got: ${tierRaw ?? "(missing)"}`,
    );
  }
...
export async function loadAgentPrompt(
  ...
  if (parsed.name !== name) {
    throw new Error(
      `agent prompt frontmatter name "${parsed.name}" does not match filename "${name}"`,
    );
  }
  return parsed;
}
 ...
 */
export async function verifyAgentPromptsAvailable(
  promptsDir = defaultPromptsDir(),
  sentinel = "architect",
): Promise<{ resolvedDir: string; sentinelFound: boolean }> {
  const names = await listAgentPrompts(promptsDir);
  const sentinelFound = names.includes(sentinel);
  if (!sentinelFound) {
    console.warn(
      `[omm-plugin] agent-prompts sentinel '${sentinel}' not found at resolved promptsDir: ${promptsDir}. ` +
        "Host bundle layout may be wrong; omm_agent_prompt_get/list will return empty.",
    );
  } else {
    console.log(
      `[omm-plugin] agent-prompts ready (resolved promptsDir: ${promptsDir}, ${names.length} prompts found)`,
    );
  }
