  ...
  it("creates an active team state with injected defaults", async () => {
    await withTmpRoot(async (root) => {
      ...
      const onDisk = JSON.parse(
        await readFile(join(root, "state", "team.json"), "utf8"),
      );
  ...
  it("merges patch onto existing active state", async () => {
    await withTmpRoot(async (root) => {
      ...
      const r = await updateModeState(
        "team",
        { fix_loop_count: 3, current_phase: "executing" },
        { stateRoot: root },
      );
      assert.equal(r.ok, true);
...
describe("cancelMode", () => {
  it("terminates with cancelled outcome by default", async () => {
    await withTmpRoot(async (root) => {
      ...
      const outcome = r.state?.outcome as { kind: string; reason: string };
  ...
  it("does not stamp phase for cancelled (validator does not accept it)", async () => {
    await withTmpRoot(async (root) => {
      ...
      assert.equal(r.state?.active, false);
      const outcome = r.state?.outcome as { kind: string };
      assert.equal(outcome.kind, "cancelled");
    });
  });
  ...
  it("is idempotent on already-terminal state", async () => {
    await withTmpRoot(async (root) => {
      ...
      const outcome = second.state?.outcome as { reason?: string };
...
describe("end-to-end mode lifecycle", () => {
  it("team: start → update → cancel(completed) full flow", async () => {
    await withTmpRoot(async (root) => {
      ...
      const b = await updateModeState(
        "team",
        { fix_loop_count: 1, current_phase: "executing" },
        { stateRoot: root },
      );
      assert.equal(b.ok, true);
      assert.equal(b.state?.fix_loop_count, 1);

      const c = await updateModeState(
        "team",
        { fix_loop_count: 2, current_phase: "verifying" },
        { stateRoot: root },
      );
