...
test("omm_agent_prompt_get returns body + structured details", async () => {
  ...
  try {
    const result = await runOmmAgentPromptGet(
      { name: "architect" },
      { promptsDir: dir },
    );
...
test("omm_agent_prompt_get rejects empty name", async () => {
  ...
  try {
    await assert.rejects(
      () => runOmmAgentPromptGet({ name: "" }, { promptsDir: dir }),
      /name.*required/i,
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
...
test("omm_agent_prompt_get rejects invalid name pattern", async () => {
  ...
  try {
    await assert.rejects(
      () => runOmmAgentPromptGet({ name: "Bad-Name" }, { promptsDir: dir }),
...
test("omm_agent_prompt_get throws when file missing", async () => {
  ...
  try {
    await assert.rejects(() =>
      runOmmAgentPromptGet({ name: "nonexistent" }, { promptsDir: dir }),
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
...
test("omm_agent_prompt_list returns empty for missing dir", async () => {
  const result = await runOmmAgentPromptList(
    {},
    { promptsDir: "/nonexistent/path/does-not-exist" },
  );
