...
import { appendProgressEntry, loadProgress, PROGRESS_FILENAME, validateProgressEntry, } from "./omm-ralph-progress.js";
...
describe("validateProgressEntry", () => {
    it("accepts a minimal valid entry", () => {
        assert.equal(validateProgressEntry({
            iteration: 0,
    ...
    it("rejects negative iteration", () => {
        assert.match(validateProgressEntry({
            iteration: -1,
            timestamp: "2026-04-26T10:00:00Z",
            summary: "x",
        }) ?? "", /non-negative/);
...
describe("appendProgressEntry / loadProgress", () => {
    it("appends and reads back one entry", async () => {
        await withTmpRoot(async (root) => {
            const r = await appendProgressEntry({ iteration: 0, summary: "started" }, root);
    ...
    it("rejects invalid entry on append (validates before writing)", async () => {
        await withTmpRoot(async (root) => {
            const r = await appendProgressEntry({ iteration: -1, summary: "bad" }, root);
...
//# sourceMappingURL=omm-ralph-progress.test.js.map