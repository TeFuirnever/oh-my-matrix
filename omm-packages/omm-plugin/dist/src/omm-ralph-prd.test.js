...
describe("validatePrd", () => {
    ...
    it("rejects story with non-boolean passes", () => {
        const r = validatePrd({
            ...minimalPrd,
            stories: [
                { id: "1", title: "x", criteria: [], passes: "false" },
            ],
        });
        assert.equal(r.ok, false);
    });
...
describe("savePrd / loadPrd", () => {
    it("saves and reads back a PRD round-trip", async () => {
        await withTmpRoot(async (root) => {
            ...
            const onDisk = JSON.parse(await readFile(join(root, "state", PRD_FILENAME), "utf8"));
    ...
    it("loadPrd reports error on malformed JSON", async () => {
        await withTmpRoot(async (root) => {
            ...
            await writeFile(join(root, "state", PRD_FILENAME), "{not valid json", "utf8");
            const r = await loadPrd(root);
            assert.equal(r.ok, false);
    ...
    it("loadPrd reports error on schema-invalid file", async () => {
        await withTmpRoot(async (root) => {
            await mkdir(join(root, "state"), { recursive: true });
            await writeFile(join(root, "state", PRD_FILENAME), JSON.stringify({ task: "x", stories: "not an array" }), "utf8");
            const r = await loadPrd(root);
            assert.equal(r.ok, false);
        });
    });
    it("savePrd rejects PRD with top-level mode field", async () => {
        await withTmpRoot(async (root) => {
            const r = await savePrd({ ...minimalPrd, mode: "ralph" }, root);
            assert.equal(r.ok, false);
            assert.match(r.error ?? "", /mode/);
        });
    });
    it("savePrd uses atomic tmp+rename (no .tmp left behind)", async () => {
        await withTmpRoot(async (root) => {
            ...
            const tmpRead = await readFile(join(root, "state", `${PRD_FILENAME}.tmp`), "utf8").catch(() => null);
...
//# sourceMappingURL=omm-ralph-prd.test.js.map