...
import { deriveOutcomeFromState, isRunOutcome, makeRunOutcome, outcomeKindToPhase, phaseToOutcomeKind, } from "./omm-run-outcome.js";
...
describe("outcomeKindToPhase", () => {
    it("is the inverse of phaseToOutcomeKind for terminal phases", () => {
        for (const k of ["completed", "failed", "blocked", "cancelled"]) {
...
describe("isRunOutcome", () => {
    ...
    it("rejects missing or wrong-typed fields", () => {
        ...
        assert.equal(isRunOutcome({ kind: "completed", mode: "ralph" }), false, "missing finishedAt");
        assert.equal(isRunOutcome({
            kind: "completed",
            mode: "ralph",
            finishedAt: "garbage",
        }), false);
...
//# sourceMappingURL=omm-run-outcome.test.js.map