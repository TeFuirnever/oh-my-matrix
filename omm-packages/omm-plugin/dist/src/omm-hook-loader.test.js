...
describe("loadHooks", () => {
    ...
    it("loads a valid hook", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "good.mjs"), validHookSource("session_start"), "utf8");
    ...
    it("filters by event name when provided", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "start.mjs"), validHookSource("session_start"), "utf8");
            await writeFile(join(dir, "end.mjs"), validHookSource("session_end"), "utf8");
    ...
    it("reports issue when event export is missing", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "bad.mjs"), "export const handler = () => {};", "utf8");
            const r = await loadHooks(dir);
    ...
    it("reports issue when handler export is missing", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "bad.mjs"), 'export const event = "ev";', "utf8");
            const r = await loadHooks(dir);
            assert.equal(r.issues.length, 1);
    ...
    it("reports issue when module fails to import", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "syntax-error.mjs"), "this is not valid javascript {{{", "utf8");
            const r = await loadHooks(dir);
            assert.equal(r.hooks.length, 0);
            assert.equal(r.issues.length, 1);
...
describe("dispatchHooks", () => {
    it("runs all matching hooks and captures values", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "h1.mjs"), `
export const event = "ev";
export const handler = async () => 1;
`, "utf8");
            await writeFile(join(dir, "h2.mjs"), `
export const event = "ev";
...
`, "utf8");
    ...
    it("captures errors per-hook without stopping others", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "throws.mjs"), `
export const event = "ev";
export const handler = async () => { throw new Error("boom"); };
`, "utf8");
            await writeFile(join(dir, "ok.mjs"), `
export const event = "ev";
...
`, "utf8");
    ...
    it("does not run hooks for other events", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "ev.mjs"), validHookSource("session_start"), "utf8");
...
`, "utf8");
    ...
    it("supports synchronous handlers", async () => {
        await withTmpDir(async (dir) => {
            await writeFile(join(dir, "sync.mjs"), `
export const event = "ev";
