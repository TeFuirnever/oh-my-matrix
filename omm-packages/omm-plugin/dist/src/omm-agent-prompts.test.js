...
import { listAgentPrompts, loadAgentPrompt, parseAgentPrompt, } from "./omm-agent-prompts.js";
const here = dirname(fileURLToPath(import.meta.url));
const BUNDLED_PROMPTS_DIR = join(here, "..", "..", "..", "omm-skills", "agent-prompts");
...
describe("parseAgentPrompt", () => {
    ...
    it("rejects missing frontmatter fence", () => {
        assert.throws(() => parseAgentPrompt("no frontmatter here"), /frontmatter fence/);
    });
    it("rejects invalid name", () => {
        assert.throws(() => parseAgentPrompt("---\nname: 1bad\nmodel_tier: sonnet\npurpose: x\n---\n\nbody"), /name must match/);
    });
    it("rejects unknown model tier", () => {
        assert.throws(() => parseAgentPrompt("---\nname: ok\nmodel_tier: gpt\npurpose: x\n---\n\nbody"), /model_tier must be one of/);
    });
    it("rejects missing purpose", () => {
        assert.throws(() => parseAgentPrompt("---\nname: ok\nmodel_tier: sonnet\n---\n\nbody"), /purpose is required/);
    });
...
describe("loadAgentPrompt + listAgentPrompts (file system)", () => {
    ...
    it("rejects when filename does not match frontmatter name", async () => {
        await withTmpDir(async (dir) => {
            ...
            await assert.rejects(() => loadAgentPrompt("wrongname", dir), /does not match filename/);
        });
    });
    it("throws on invalid name passed to loadAgentPrompt", async () => {
        await assert.rejects(() => loadAgentPrompt("../escape"), /invalid agent prompt name/);
    });
