...
import { dispatchOmmHooks, handleModeChange, handlePostToolUse, handlePreToolUse, handleSessionEnd, handleSessionStart, } from "./omm-hooks.js";
describe("omm-hooks event sources", () => {
    ...
    it("dispatchOmmHooks loads and runs a user hook", async () => {
        ...
        await writeFile(hookPath, `export const event = "post_tool_use";
       import { writeFileSync } from "node:fs";
       export async function handler(args) {
         writeFileSync(${JSON.stringify(sentinelPath)}, JSON.stringify(args));
       }`, "utf8");
        const outcome = await dispatchOmmHooks("post_tool_use", { toolName: "omm_ping", durationMs: 5 }, { stateRoot });
        assert.ok(outcome);
    ...
    it("dispatchOmmHooks swallows handler errors (other hooks still run)", async () => {
        ...
        await mkdir(dir, { recursive: true });
        await writeFile(join(dir, "thrower.mjs"), `export const event = "pre_tool_use";
       export async function handler() { throw new Error("boom"); }`, "utf8");
        await writeFile(join(dir, "ok.mjs"), `export const event = "pre_tool_use";
    ...
    it("handlePreToolUse / handlePostToolUse / handleModeChange dispatch their respective events", async () => {
        const events = ["pre_tool_use", "post_tool_use", "mode_change"];
