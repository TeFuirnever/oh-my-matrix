...
import { runOmmStateList, runOmmStateRead, runOmmStateWrite, sanitizeStateKey, } from "./omm-state.js";
...
describe("omm_state_write", () => {
    ...
    it("rejects missing key", async () => {
        await withTmpDir(async (dir) => {
            const r = await runOmmStateWrite({ value: { foo: 1 } }, { stateRoot: dir });
...
describe("omm_state_read", () => {
    ...
    it("returns null for missing key", async () => {
        await withTmpDir(async (dir) => {
            const r = await runOmmStateRead({ key: "nonexistent" }, { stateRoot: dir });
...
describe("path traversal defense (integration)", () => {
    it("write rejects ../ keys without touching filesystem", async () => {
        await withTmpDir(async (dir) => {
            const r = await runOmmStateWrite({ key: "../escape", value: { foo: 1 } }, { stateRoot: dir });
            assert.ok(r.content[0].text.includes("error"));
