...
import { __resetKeyLocksForTest, withCrossProcessLock, withKeyLock, } from "./omm-fs-queue.js";
describe("withKeyLock", () => {
    ...
    it("propagates the awaited result and error to its own caller", async () => {
        ...
        await assert.rejects(withKeyLock("k", async () => {
...
describe("withCrossProcessLock", () => {
    ...
    it("retries past EEXIST until the prior holder releases", async () => {
        ...
        try {
            ...
            await wf(lockPath, `${JSON.stringify({ pid: process.pid, startedAt: new Date().toISOString(), hostname: "self" })}\n`, { flag: "wx" });
            const released = (async () => {
                await new Promise((r) => setTimeout(r, 120));
                order.push("released");
                await unlink(lockPath);
            })();
            const acquired = withCrossProcessLock(dir, "k", async () => {
                order.push("ran");
                return "ok";
            }, { timeoutMs: 2000 });
    ...
    it("reclaims a stale lock whose recorded PID is dead", async () => {
        ...
        try {
            ...
            await mkdir(join(dir, ".locks"), { recursive: true });
            const lockPath = join(dir, ".locks", "k.lock");
            await writeFile(lockPath, `${JSON.stringify({ pid: -1, startedAt: new Date(0).toISOString(), hostname: "ghost-host-not-this-machine" })}\n`);
            const oldTime = new Date(Date.now() - 60_000);
            await utimes(lockPath, oldTime, oldTime);
            const value = await withCrossProcessLock(dir, "k", async () => "reclaimed", { staleMs: 5_000, timeoutMs: 2_000 });
    ...
    it("throws OMM_E_LOCK_TIMEOUT when the lock cannot be acquired in time", async () => {
        ...
        try {
            ...
            await mkdir(join(dir, ".locks"), { recursive: true });
            const lockPath = join(dir, ".locks", "k.lock");
            await writeFile(lockPath, `${JSON.stringify({ pid: process.pid, startedAt: new Date().toISOString(), hostname: "self" })}\n`);
            await assert.rejects(withCrossProcessLock(dir, "k", async () => "should-not-run", {
                timeoutMs: 200,
                staleMs: 60_000,
    ...
    it("releases the lock even when fn throws (try/finally cleanup)", async () => {
        ...
        try {
            const lockPath = join(dir, ".locks", "k.lock");
            await assert.rejects(withCrossProcessLock(dir, "k", async () => {
...
//# sourceMappingURL=omm-fs-queue.test.js.map