...
import { advanceStage, getCurrentStage, incrementRetry, markStageStatus, validatePlan, } from "./omm-autopilot-pipeline.js";
const samplePlan = [
...
const stateWith = (plan, current_step = 1) => ({
    mode: "autopilot",
    active: true,
    plan,
    current_step,
...
describe("getCurrentStage", () => {
    ...
    it("returns null when plan is malformed", () => {
        const malformed = stateWith([{ step: 0, description: 1, status: "pending", retries: 0 }], 0);
...
describe("markStageStatus", () => {
    ...
    it("attaches optional summary", () => {
        const r = markStageStatus(stateWith(samplePlan, 1), 1, "complete", "tests pass");
        const updated = r.patch?.plan;
    ...
    it("rejects invalid status string", () => {
        const r = markStageStatus(stateWith(samplePlan, 1), 1, "bogus");
        assert.equal(r.ok, false);
    });
...
describe("incrementRetry", () => {
    ...
    it("does not enforce a cap (caller owns policy)", () => {
        ...
        assert.equal(r.ok, true);
        assert.equal((r.patch?.plan)[0].retries, 100);
    });
});
describe("advanceStage", () => {
    it("advances when current stage is complete", () => {
        ...
        const updated = r.patch?.plan;
        assert.equal(updated[1].status, "in_progress", "next stage marked in_progress");
    });
    ...
    it("advances past the last stage and omits plan patch", () => {
        ...
        assert.equal(r.ok, true);
        assert.equal(r.patch?.current_step, 1);
        assert.equal(r.patch?.plan, undefined, "no further stage to mark in_progress");
    });
...
//# sourceMappingURL=omm-autopilot-pipeline.test.js.map