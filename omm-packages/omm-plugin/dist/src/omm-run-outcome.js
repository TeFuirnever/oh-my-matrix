 ...
 */
const VALID_KINDS = new Set(["completed", "failed", "blocked", "cancelled"]);
const VALID_MODES = new Set(["ralph", "autopilot", "team"]);
const KIND_TO_PHASE = {
  completed: "complete",
  failed: "failed",
  blocked: "blocked",
...
const PHASE_TO_KIND = Object.fromEntries(
  Object.entries(KIND_TO_PHASE).map(([kind, phase]) => [phase, kind]),
);
/**
...
export function isRunOutcome(value) {
  ...
  if (typeof v.mode !== "string" || !VALID_MODES.has(v.mode)) return false;
