    ...
    describe("Resources (omm://state/<key>)", () => {
        it("resources/list returns written state files as omm://state/<key>", async () => {
            await withClient(async (client) => {
                ...
                const resources = r.result
                    .resources;
                const uris = resources.map((x) => x.uri);
                assert.ok(uris.includes("omm://state/ralph"), `expected omm://state/ralph in ${JSON.stringify(uris)}`);
            });
        });
    ...
    describe("Prompts (agent-prompts/<name>.md)", () => {
        it("prompts/list includes the sentinel prompts (analyst + planner)", async () => {
            await withClient(async (client) => {
                ...
                const prompts = r.result.prompts.map((p) => p.name);
                assert.ok(prompts.includes("analyst"), `expected 'analyst' in ${JSON.stringify(prompts)}`);
                assert.ok(prompts.includes("planner"), `expected 'planner' in ${JSON.stringify(prompts)}`);
            });
        });
        it("prompts/get returns the prompt body as a system message", async () => {
            await withClient(async (client) => {
                ...
                assert.ok(result.messages[0].content.text.length > 100, "prompt body too short");
            });
        });
